...
public class RamRegistry implements DbContext {
    private final HashMap<UUID, Integer> access_tokens = new HashMap<>();
    ...
    @Override
    public Optional<TokenPassPair> find_password_token_pair_by_username(String username) {
        return Optional.ofNullable(tokens.get(username));
    }

    @Override
    public void insert_password_token_pair(String username, TokenPassPair password_token_pair) {
        tokens.put(username, password_token_pair);
    }

    @Override
    public Optional<AccountInfo> find_account_info_from_id(int account_id) {
        return account_id < 0 || account_id >= accounts.size() ? Optional.empty() : Optional.ofNullable(accounts.get(account_id));
    }

    @Override
    public int insert_new_account(AccountInfo account_info) {
    ...
    @Override
    public void insert_new_access_token(UUID access_token, int id) {
        access_tokens.put(access_token, id);
    }

    @Override
    public int find_account_id_from_access_token(UUID access_token) {
        var out = access_tokens.get(access_token);
        return out == null ? -1 : out;
    }

    @Override
    public AccountInfo[] get_all_accounts() {
        return accounts.toArray(new AccountInfo[0]);
    }

    @Override
    public int insert_new_restaurant(RestaurantInfo info) {
        restaurants.add(new Restaurant(info, new ArrayList<>()));
        return restaurants.size()-1;
    }

    @Override
    public RestaurantInfo[] get_all_restaurants() {
        return restaurants.stream().map((rest) -> rest.info).toArray(RestaurantInfo[]::new);
    }

    @Override
    public void insert_new_product_for_restaurant(int restaurant_id, ProductInfo product_info) {
        restaurants.get(restaurant_id).products.add(product_info);
    }

    @Override
    public Optional<ProductInfo[]> get_all_products_for_restaurant(int restaurant_id) {
    ...
    @Override
    public void insert_new_order(int issued_by_whom, List<OrderItem> order) {
        orders.add(new Order(false, -1, issued_by_whom, order));
    }

    @Override
    public PendingOrder[] get_pending_orders() {
    ...
    @Override
    public boolean accept_order(int accepted_by_whom, int id) {
